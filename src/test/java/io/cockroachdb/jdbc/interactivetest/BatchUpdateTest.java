...
public class BatchUpdateTest extends AbstractIntegrationTest {
    ...
    private List<Product> findAll(int limit) throws SQLException {
        return JdbcTemplate.from(dataSource)
                .queryForList("select * from product order by id limit " + limit,
                        (rs, rowNum) -> {
    ...
    @Order(1)
    @ParameterizedTest
    @ValueSource(ints = {1 << 4, 1 << 5, 1 << 6, 1 << 7, 1 << 8, 1 << 9, 1 << 10})
    public void whenUpdateUsingBatches_thenObserveSingletons(int batchSize) throws Exception {
        ...
        try (Connection connection = dataSource.getConnection()) {
            ...
            chunks.forEach(chunk -> {
                System.out.printf("\r%s", AsciiText.progressBar(totalChunks, n.incrementAndGet(), batchSize + ""));
